...
class _DemandesScreenState extends State<DemandesScreen> {
  ...
  Future<void> _load() async {
    ...
    _setupRealtimeListener(user.id, user.role == 'admin');
  }

  void _setupRealtimeListener(String userId, bool isAdmin) {
    _demandesSubscription?.cancel();
    _demandesSubscription = _apiService.watchDemandes(
      ...
      onError: (error) {
        debugPrint('Error listening to demandes: $error');
        if (mounted) {
          setState(() {
            _isLoading = false;
          });
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'ID: ${demande.id.length > 6 ? demande.id.substring(0, 6).toUpperCase() : demande.id.toUpperCase()}',
                                style: TextStyle(fontSize: 10, color: AppColors.onSurfaceVariant.withOpacity(0.5), fontWeight: FontWeight.bold),
                              ),
                            ],
                          ),
                        ),
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  _DetailRow(icon: LucideIcons.fileText, label: 'Motif', value: demande.motif),
                  if (demande.description != null && demande.description!.isNotEmpty)
                    _DetailRow(icon: LucideIcons.alignLeft, label: 'Description', value: demande.description!),
                  _DetailRow(
